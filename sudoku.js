...
if (Meteor.is_client) {
  ...
  Template.say.has_current_player = Template.form.has_current_player = function(){
  ...
  Template.say.events = {
    "submit": function(event){
      event.preventDefault();
      var say = $(event.target)
      Messages.insert({content: say.find('input#content').val(), player: current_player,time: new Date()});
